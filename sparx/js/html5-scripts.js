...
$(function() {
    $(window).on('resize', function resize()  {
        ...
        setTimeout(function () {
            var content = $('.content');
            var banner = $('.banner');
             var top = (window.innerHeight - (content.height()+ banner.height())) / 2;
...
return this.each(function(){var el=$(this);var array=[];$.each(el.html().split(settings.separator),function(key,value){array.push(value)});el.html(array[0]);
var rotate=function(){switch(settings.animation){case"dissolve":el.animate({textShadowBlur:0,opacity:0},500,function(){index=$.inArray(el.html(),array);
if((index+1)==array.length){index=-1}el.html(array[index+1]).animate({textShadowBlur:0,opacity:1},500)});
;break;case"fade":el.fadeOut(settings.speed,function(){index=$.inArray(el.html(),
array);if((index+1)==array.length){index=-1}el.html(array[index+1]).fadeIn(settings.speed)});break}};setInterval(rotate,settings.speed*2)})}}
(window.jQuery);
